...
Page({
  ...
  locationCorrect: function (latitude, longitude) {
    var that = this
    var url = "https://api.map.baidu.com/geoconv/v1/";//访问 web 端百度API 经纬度转换方法
    //访问页面所需要提供的参数
    var params = {
      from: 1,
      to: 5,
      coords: longitude + "," + latitude,
      ak: "umHlcGePUGYk1DwtVHIdT6zWueQ6SgcD" //百度网页端服务 AK
    }
    //调用微信访问 url 方法
    wx.request({
      url: url,
  ...
  getCity: function (latitude, longitude) {
    var that = this
    var url = "https://api.map.baidu.com/geocoder/v2/";//访问 web 端百度API 逆经纬度转换方法
    var params = {
      ak: "umHlcGePUGYk1DwtVHIdT6zWueQ6SgcD",//百度网页端服务 AK（貌似微信小程序 AK 也行）
      output: "json",
  ...
  getWeahter: function (city) {
    var that = this
    var url = "https://api.map.baidu.com/telematics/v3/weather"//访问百度API 微信小程序端获取天气方法
    var params = {
      location: city,
      output: "json",
      ak: "umHlcGePUGYk1DwtVHIdT6zWueQ6SgcD"//百度微信小程序 AK
    }
    wx.request({
      url: url,
      data: params,
 ...
 var pm25 = res.data.results[0].pm25;//PM2.5
        ...
        var pic2 =
          res.data.results[0].weather_data[0].nightPictureUrl;//天气图片
